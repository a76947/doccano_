...
class CommentList(generics.ListCreateAPIView):
    """
    Lista comentários ou cria um novo.
    Query params: ?example=<id>&label=<label_id>&limit=…&offset=…
    """

    permission_classes = [IsAuthenticated & IsProjectMember]
    serializer_class   = CommentSerializer
    filter_backends    = (DjangoFilterBackend, filters.SearchFilter, filters.OrderingFilter)
    filterset_fields   = ["example", "label"]
    search_fields      = ("text",)
    ordering_fields    = ("created_at", "example")

    # GET
    def get_queryset(self):
        return Comment.objects.filter(example__project_id=self.kwargs["project_id"])

    # POST
    def perform_create(self, serializer):
        label_param = self.request.data.get("label")
        label_id = int(label_param) if label_param not in (None, "", "null") else None
    ...
    def delete(self, request, *args, **kwargs):
        ...
        Comment.objects.filter(user=request.user, pk__in=ids).delete()
...
class BaseListAPI(generics.ListCreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({"detail": err.messages},
